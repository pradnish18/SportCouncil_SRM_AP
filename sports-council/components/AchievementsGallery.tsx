...
const staticAchievements = [
    { id: "1", title: "South Zone Cricket Gold", description: "SRM AP claimed Gold at the South Zone Inter-University Cricket Championship.", sport: "Cricket", date: "2025-03-14", category: "TROPHY", photoUrl: "https://images.unsplash.com/photo-1578269174936-2709b6aeb913?q=80&w=2071&auto=format&fit=crop" },
    { id: "2", title: "State Football Runners-up", description: "Our football team reached the state finals in an outstanding campaign.", sport: "Football", date: "2024-11-20", category: "TROPHY", photoUrl: "https://images.unsplash.com/photo-1574629810360-7efbbe195018?q=80&w=2093&auto=format&fit=crop" },
    { id: "3", title: "Kabaddi Intra-Hostel Trophy", description: "Dominant performance throughout the hostel league season.", sport: "Kabaddi", date: "2025-01-15", category: "TROPHY", photoUrl: "https://images.unsplash.com/photo-1544652478-6653e09f18a2?q=80&w=2070&auto=format&fit=crop" },
    { id: "4", title: "Zonal Chess Championship", description: "Claimed all top positions at the Zonal Chess Championship.", sport: "Chess", date: "2023-08-05", category: "TROPHY", photoUrl: "https://images.unsplash.com/photo-1580541832626-2a7131ee809f?q=80&w=2072&auto=format&fit=crop" },
    { id: "5", title: "Alex James — 100m Sprint Record", description: "Alex set a new university record of 10.4 seconds in the 100m Sprint.", sport: "Athletics", date: "2024-03-10", category: "RECORD", photoUrl: "" },
    { id: "6", title: "Sarah Khan — Highest Run Scorer", description: "Sarah scored the most runs across the entire EVC cricket tournament.", sport: "Cricket", date: "2025-02-08", category: "ACCOLADE", photoUrl: "" },
    { id: "7", title: "Badminton Women's Gold", description: "Women's doubles team won Gold at the inter-collegiate tournament.", sport: "Badminton", date: "2025-02-25", category: "TROPHY", photoUrl: "https://images.unsplash.com/photo-1626225453014-490396593447" },
    { id: "8", title: "Table Tennis South Zone", description: "SRM AP swept Gold and Silver at the South Zone table tennis.", sport: "Table Tennis", date: "2024-09-12", category: "TROPHY", photoUrl: "https://images.unsplash.com/photo-1534158914592-062992fbe900" },
];

const CATEGORY_ICONS: Record<string, string> = { TROPHY: "🏆", ACCOLADE: "🥇", RECORD: "📋" };
...
export default function AchievementsGallery() {
    ...
    const allAchievements = useMemo(() => {
        ...
        return raw
            ...
            .sort((a: any, b: any) => b.dateObj - a.dateObj); // reverse chronological
    }, [apiData]);

    const uniqueSports = useMemo(() => ["All", ...Array.from(new Set(allAchievements.map((a: any) => a.sport as string)))], [allAchievements]);
    const uniqueYears = useMemo(() => ["All", ...Array.from(new Set(allAchievements.map((a: any) => a.year as string))).sort((a, b) => Number(b) - Number(a))], [allAchievements]);

    const filtered = useMemo(() => allAchievements.filter((a: any) => {
    ...
    const trophies = filtered.filter((a: any) => a.category === 'TROPHY');
    const accolades = filtered.filter((a: any) => a.category === 'ACCOLADE');
    const records = filtered.filter((a: any) => a.category === 'RECORD');
        ...
        <div className="space-y-32">
            ...
            <div className="flex flex-wrap gap-4 items-center p-8 rounded-[2.5rem] bg-card border border-border shadow-sm">
                ...
                <select value={selectedSport} onChange={e => setSelectedSport(e.target.value)}
                    ...
                    {uniqueSports.map(s => <option key={s as string} value={s as string}>{s as string}</option>)}
                </select>
                {/* Year filter */}
                <select value={selectedYear} onChange={e => setSelectedYear(e.target.value)}
                    className="bg-background border border-border text-foreground text-sm font-bold rounded-xl px-5 py-2.5 focus:outline-none focus:ring-2 focus:ring-brand-srm/20 focus:border-brand-srm transition-all cursor-pointer">
                    {uniqueYears.map(y => <option key={y as string} value={y as string}>{y as string}</option>)}
                </select>
                {/* Category pills */}
                <div className="flex flex-wrap gap-2 ml-2">
                    {["All", "TROPHY", "ACCOLADE", "RECORD"].map(c => (
                        <button key={c} onClick={() => setSelectedCategory(c)}
                            className={`px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border ${selectedCategory === c ? 'bg-brand-srm text-white border-brand-srm shadow-lg shadow-brand-srm/30' : 'border-border text-muted hover:text-foreground hover:border-brand-srm'}`}>
                            {c === "All" ? "All" : `${CATEGORY_ICONS[c]} ${c}`}
                        </button>
                    ))}
                </div>
                {(selectedSport !== "All" || selectedYear !== "All" || selectedCategory !== "All") && (
                    <button onClick={() => { setSelectedSport("All"); setSelectedYear("All"); setSelectedCategory("All"); }}
                ...
                <section>
                    ...
                    <div className="columns-1 md:columns-2 lg:columns-3 gap-8 space-y-8">
                        {trophies.map((trophy: any, idx: number) => (
                            ...
                            >
                                {trophy.photoUrl ? (
                                    <Image src={trophy.photoUrl} alt={trophy.title} fill className="object-cover grayscale group-hover:grayscale-0 transition-all duration-1000 scale-110 group-hover:scale-100" sizes="(max-width: 768px) 100vw, 33vw" />
                                ) : (
                                    <div className="absolute inset-0 bg-brand-srm/5 flex items-center justify-center text-7xl opacity-40">🏆</div>
                                )}
                                ...
                                <div className="absolute inset-0 p-10 flex flex-col justify-end translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                                    ...
                                    <h3 className="text-2xl font-syne font-black text-foreground leading-tight group-hover:text-brand-srm transition-colors mb-3">{trophy.title}</h3>
            ...
            {accolades.length > 0 && (
                <section className="pt-32 border-t border-border">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-10">
                        {accolades.map((a: any, idx: number) => (
                            <motion.div key={a.id}
                                ...
                                {a.photoUrl && (
                                    <div className="relative h-56 w-full overflow-hidden">
                                        <Image src={a.photoUrl} alt={a.title} fill className="object-cover group-hover:scale-110 transition-transform duration-1000" />
                                ...
                                <div className="p-10 flex flex-col flex-1">
                                    ...
                                    <h4 className="text-2xl font-syne font-black text-foreground mb-4 leading-tight group-hover:text-brand-srm transition-colors">{a.title}</h4>
                        ...
                        {records.map((r: any, idx: number) => (
                            <motion.div key={r.id}
                                ...
                                <div className="flex-1 min-w-0 text-center sm:text-left">
                                    <h4 className="text-2xl font-syne font-black text-foreground mb-3 leading-tight group-hover:text-brand-srm transition-colors">{r.title}</h4>
                                    <p className="text-sm text-muted font-outfit mb-6 leading-relaxed">{r.description}</p>
                                    <div className="flex flex-wrap justify-center sm:justify-start gap-3">
                                        <span className="text-[9px] font-black uppercase tracking-[0.15em] text-brand-srm bg-brand-srm/5 px-3 py-1.5 rounded-lg border border-brand-srm/10">{r.sport}</span>
                                        <span className="text-[9px] font-black uppercase tracking-[0.15em] text-muted bg-foreground/[0.03] px-3 py-1.5 rounded-lg border border-border">{r.year}</span>
                                    </div>
                                </div>
                            </motion.div>
                        ))}
