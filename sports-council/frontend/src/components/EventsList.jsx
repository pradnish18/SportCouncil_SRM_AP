...
const getSportIcon = (sport) => SPORTS_ICONS[sport] || "🏆";
...
const staticEvents = [
  {
    id: "1",
    title: "Inter-Hostel Cricket League",
    sport: "Cricket",
    venue: "Hostel Ground",
    date: "2026-03-15",
    stage: "LIVE",
    category: "Hostel vs Hostel",
    team1: "Hostel 1",
    team2: "Hostel 4",
    score1: "145/6",
    score2: "Batting",
    liveUpdates: "Hostel 1 batting powerfully in 15th over",
  },
  {
    id: "2",
    title: "University Open Badminton",
    sport: "Badminton",
    ...
    description: "Annual badminton championship open to all university students.",
  },
  {
    id: "3",
    ...
    stage: "UPCOMING",
    time: "07:30 AM",
    category: "Intra-University",
    description: "Track and field events for all departments. Registration open for 100m, 200m, relay, high jump.",
  },
  {
    id: "4",
    title: "South Zone Football Cup",
    sport: "Football",
    venue: "Main Ground",
    date: "2026-04-05",
    stage: "UPCOMING",
    time: "04:00 PM",
    category: "Inter-University",
    description: "SRM AP hosts the South Zone Inter-University Football Cup. Support our team!",
  },
  {
    id: "5",
    ...
    stage: "PAST",
    winner1st: "SRM Blue",
    winner2nd: "Vellore United",
    matchDetails: "SRM won by 40 runs in a thrilling final. Rahul scored a notable 85.",
  },
  {
    id: "6",
    title: "Intra-School Chess Cup",
    sport: "Chess",
    venue: "Admin Hall",
    date: "2026-02-18",
    stage: "PAST",
    winner1st: "Rahul S.",
    winner2nd: "Priya V.",
    matchDetails: "Exhilarating endgame by Rahul clinched the title in 48 moves.",
  },
  {
    id: "7",
    title: "Kabaddi Zonal Meet",
    sport: "Kabaddi",
    venue: "Sports Complex",
    date: "2026-01-30",
    stage: "PAST",
    winner1st: "SRM Raiders",
    winner2nd: "VITU Panthers",
    matchDetails: "SRM Raiders dominated the final with strong raiding performance.",
  },
];
...
export default function EventsList() {
  ...
  const uniqueSports = useMemo(
    () => ["All", ...Array.from(new Set(formattedEvents.map((e) => e.sport)))],
    [formattedEvents]
  );

  const liveEvents = useMemo(() => formattedEvents.filter((e) => e.stage === "LIVE"), [formattedEvents]);
        ...
        <div className="flex gap-2 p-1.5 bg-background border border-border rounded-2xl w-full lg:w-fit">
          {["LIVE", "UPCOMING", "PAST"].map((stage) => (
            <button
              ...
              className={`relative flex-1 lg:flex-none px-8 py-3 rounded-xl text-[10px] font-black tracking-[0.2em] uppercase transition-all ${
                filter === stage ? "bg-brand-srm text-white shadow-xl shadow-brand-srm/20" : "text-muted hover:text-foreground"
              }`}
            >
              {stage}
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className={`grid gap-8 ${
            filter === "PAST" ? "grid-cols-1 md:grid-cols-2" : "grid-cols-1 md:grid-cols-2 lg:grid-cols-3"
          }`}
        >
          {paginated.map((event) => (
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
              <button
                ...
                className={`w-14 h-14 rounded-2xl text-sm font-black transition-all border ${
                  page === p ? "bg-brand-srm border-brand-srm text-white shadow-lg" : "bg-card border-border text-muted hover:border-brand-srm hover:text-foreground"
                }`}
              >
                {p}
              </button>
...
function EventCard({ event }) {
  ...
  return (
    <motion.article
      ...
      className={`flex flex-col rounded-[2.5rem] overflow-hidden theme-card border group transition-all duration-300 ${
        isLive ? "border-red-500/30 shadow-red-500/5" : "border-border hover:border-brand-srm/40 shadow-xl shadow-foreground/5"
      }`}
    >
      {/* Past event: photo strip at top */}
      {isPast && event.photoUrl && (
        <div className="relative h-48 w-full overflow-hidden">
          <img src={event.photoUrl} alt={event.title} className="absolute inset-0 object-cover group-hover:scale-110 transition-transform duration-700" />
      ...
      <div className="p-8 flex flex-col flex-1">
        ...
        <h3 className="text-2xl font-syne font-black text-foreground mb-6 leading-tight group-hover:text-brand-srm transition-colors">
          {event.title}
        </h3>
        ...
        {isPast && event.winner1st && (
          <div className="mt-auto pt-6 border-t border-border">
            ...
            <div className="space-y-3">
              ...
              {event.matchDetails && (
                <p className="text-[11px] text-muted font-outfit pt-2 italic opacity-80 leading-relaxed text-center">
                  "{event.matchDetails}"
                </p>
              )}
            </div>
