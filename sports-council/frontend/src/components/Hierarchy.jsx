...
export default function Hierarchy() {
  ...
  const clubs = clubsArr.map((club) => {
    ...
    return {
      ...
      icon,
      image: club.bgImageUrl || "https://images.unsplash.com/photo-1531415074968-036ba1b575da?q=80&w=2067&auto=format&fit=crop",
  ...
  return (
    <section className="py-24 px-6 max-w-7xl mx-auto border-t border-border bg-background transition-colors duration-500">
      ...
      >
        <div className="inline-block px-4 py-1.5 mb-6 text-[10px] font-bold tracking-[0.3em] uppercase border rounded-xl border-brand-srm/20 bg-brand-srm/5 text-brand-srm">
          Leadership
        </div>
        <h2 className="text-4xl md:text-5xl font-syne font-extrabold text-foreground tracking-tight">Council Hierarchy</h2>
        <p className="mt-4 text-muted font-outfit max-w-xl mx-auto">
          The leadership steering SRM Sports towards competitive excellence and holistic development.
        </p>
      </motion.div>
      ...
      {sportsCouncil.length > 0 && (
        <div className="mb-16 relative py-12">
          ...
          <div className="relative flex justify-center">
            <span className="bg-background px-8 font-syne font-bold text-3xl text-foreground transition-colors duration-500">
              Sports Council
            </span>
          </div>
        </div>
...
function MemberCard({ member }) {
  return (
    <motion.div variants={memberCardVariants} className="group">
      ...
      <h4 className="text-lg sm:text-xl font-syne font-extrabold text-foreground group-hover:text-brand-srm transition-colors leading-tight mb-1">
        {member.name}
      </h4>
